...
The key is the visible protocol: `ASSIGN → VERIFIER → DONE or BLOCKED`. The
Verifier is a code gate, not a promise that every workspace has another agent
seat. Recording mode also runs two fake unsafe requests—send a customer email
...
- The account timeline and reply handler are implemented, opt-in, and not part
  of the demo claim.
