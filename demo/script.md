...
The Tier 1 protocol is visible as posts in `#agents-floor`: Desk `ASSIGN`, then `VERIFIER · approved | refused`, then worker `DONE | BLOCKED`, followed by a one-item Ember Grill brief in `#attention`. The Verifier is a code gate whose verdict appears from Desk. There is no Verifier or Closer seat.
...
- `FINDING · Follow-up`
- eight `FINDING · Follow-up` cards; there are no Ops or Inbox findings in this round
